...
async function getCurrentLocation(name) {
  try {
    ...
    const object = {
      city: location.name,
      country:
        location.sys.country !== undefined
          ? regionNamesInEnglish.of(location.sys.country)
          : '',
      ...
      sunrise: addSeconds(fromUnixTime(location.sys.sunrise), timezone),
      sunset: addSeconds(fromUnixTime(location.sys.sunset), timezone),
...
async function getForecastData(latitude, longitude) {
  let forecastData = await fetch(
    `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${latitude},${longitude}/${format(
      new Date(),
      'yyyy-MM-dd-hh',
    )}/${format(
      addDays(new Date(), 6),
