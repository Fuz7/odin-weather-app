...
function setActiveCard(dayType){
  const weekWeatherCard = Array.from(document.getElementsByClassName('weekWeatherCard'))
...
function updateForecastCard(forecastData){
  ...
  formattedData.forEach((data, index)=>{
    const imageContainer = document.getElementsByClassName('weatherType')[index]
    imageContainer.className = 'weatherType'
    setForecastWeatherTypeHtml(imageContainer,data.icon)
    const weatherTemp = document.getElementsByClassName('weekWeatherTemp')[index]
...
function updateCurrentCard(location) {
  ...
  const headerData = (location.city !== location.country && location.country !== '') ?`${location.city}, ${location.country}`: location.city
